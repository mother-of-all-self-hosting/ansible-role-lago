...
#
# Tags look like `v<Lago version>-<release>`:
#
# - if defaults/main.yml points at a Lago version that has never been released,
#   the release counter restarts at 0 (`v1.51.0-0`)
# - otherwise the counter is incremented (`v1.51.0-1`), but only if something
...
#
# The role installs several components, but only `lago_version` names the tag.
# It is the one upstream releases under, and it drives both the API and the
# frontend image. A bump of `lago_gotenberg_version` still earns a release -
# it changes defaults/main.yml - it just gets the next counter under the
# current Lago version rather than a version of its own.
...
# Anchored on `lago_version:` so that neither `lago_gotenberg_version:` nor a
# commented-out line can be mistaken for it.
version="$(sed -nE 's|^lago_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Lago version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.51.0`),
